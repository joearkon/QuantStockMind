    ...
    candidates: {
      ...
      items: {
        ...
        properties: {
          ...
          elasticity_score: { type: Type.NUMBER }, // 弹性爆发分
          market_cap_label: { type: Type.STRING }, // 市值描述
          catch_up_anchor_leader: { type: Type.STRING }, // 参照龙头
...
export const fetchHotMoneyAmbush = async (apiKey: string): Promise<AnalysisResult> => {
  ...
  const prompt = `
    ...
    [!!! 选股偏好：弹性 > 规模 !!!]:
    1. **拒绝“重坦”**：避开浪潮信息、工业富联、中际旭创等千亿以上大市值。这些票适合百万级稳健资金，不适合 8 万复利。
    2. **锁定“活跃小马”**：寻找市值在 50亿 - 200亿 之间，历史上有过 3 连板以上基因的标的。
    3. **补涨逻辑对齐**：寻找对应板块领涨龙头的“底部影子股”。
       - Sanhua Intelligent (三花智控) 对应 -> 上海机电 (底部补涨逻辑)
       - Inspur Info (浪潮信息) 对应 -> 四川九洲 (高弹性活跃品种)
    4. **形态位置审计**：重点探测从高位回撤 35%+、目前在 120 日线附近极致缩量横盘或构筑圆弧底的标的。
    
    [!!! 数据纠偏指令 !!!]:
    1. 严禁返回旧价格（如国轩高科 28 这种过时数据）。
    2. 如果价格数据存疑，在 turnaround_logic 中注明“以低位补涨逻辑为准，价格参考实盘形态”。
