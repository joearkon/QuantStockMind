    ...
    candidates: {
      ...
      items: {
        ...
        properties: {
          ...
          sector_heat_status: { type: Type.STRING, enum: ['Ice', 'Warm', 'Boiling'] },
          catalyst_jan_strength: { type: Type.NUMBER },
...
export const fetchMarketDashboard = async (period: 'day' | 'month', market: MarketType, apiKey?: string): Promise<AnalysisResult> => { const ai = new GoogleGenAI({ apiKey: apiKey || process.env.API_KEY }); const now = new Date(); const timeContext = now.toLocaleString('zh-CN'); let indexNames = ""; if (market === MarketType.CN) indexNames = "上证指数, 深证成指, 创业板指, 科创50, 沪深300"; else if (market === MarketType.HK) indexNames = "恒生指数, 恒生科技指数"; else if (market === MarketType.US) indexNames = "标普500, 纳斯达克, 道琼斯"; const prompt = `【绝对任务：全量资金成分审计】当前时间: ${timeContext}。作为首席量化分析师，利用 googleSearch 检索 ${market} 的实时盘面数据。[重点探测逻辑]：1. **沪指连阳状态确认**：确认当前是否处于极长的连阳周期（如 15-17 连阳），并搜索市场对“连阳终结”的共识。2. **四路资金拆解**：- **外资**：检索北向资金今日净流入/流出金额及主要介入行业。- **国内机构**：检索主流基金、社保、险资的加仓动向或分歧信号。- **顶级游资**：审计龙虎榜中知名席位（呼家楼、章盟主等）的净买入额。- **散户热度**：重点检索“拉萨天团”活跃度、开户数变动、社交媒体（东方财富股吧、雪球）的人气排名。3. **过热预警评分**：如果散户大量进场且融资余额飙升，请在 warning_level 中标记为 Overheated 或 Extreme。目标指数: ${indexNames}。输出 JSON。`; const response = await ai.models.generateContent({ model: GEMINI_MODEL_PRIMARY, contents: prompt, config: { tools: [{ googleSearch: {} }], responseMimeType: "application/json", responseSchema: marketDashboardSchema } }); const parsed = robustParse(response.text || "{}"); return { content: response.text || "", timestamp: Date.now(), modelUsed: ModelProvider.GEMINI_INTL, isStructured: true, structuredData: parsed, market }; };
