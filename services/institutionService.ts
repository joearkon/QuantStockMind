...
export const fetchHighFreqSurveys = async (
  ...
  const prompt = `
    作为资深投研分析师，利用 googleSearch 检索 ${market} 市场【最近 1 周内（即截至 ${new Date().toLocaleDateString('zh-CN')}）】机构调研的真实数据。
    
    【核心目标与真实性要求】
    1. **时间对齐**：必须筛选明确标注为“近期”或“本周”的数据，严禁混入 1 个月前的历史过期信息。
    2. **高频标的**：筛选被机构调研“最频繁、参与机构数量最集中”的公司排行榜。
    3. **逻辑穿透**：必须指出标的被调研的核心基本面原因（如：产能扩张、业绩拐点、新产品发布）。
